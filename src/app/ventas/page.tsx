...
export default function VentasPage() {
  ...
  const iniciar = async () => {
    ...
    const { data: vend } = await supabase
      .from('usuarios').select('id, nombre').eq('rol', 'ventas').eq('activo', true).order('nombre')
    setVendedoras(vend || [])
